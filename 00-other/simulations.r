...
smat <- matrix(nrow=10,ncol=1)

for(s in 1:10){
  n <- s*10
  
  tmat <- matrix(nrow=S,ncol=1)
  ...
  for(i in 1:S){
    ...
    tmat[i,] <- summary(lm(d~1))$coefficients[3]
  }
  
  tmat_dt <- data.table(tmat)
  colnames(tmat_dt) <- "trat"
  
  tr <- qt(p=.025,df=n-1,lower.tail=F)
  
  smat[s,] <- nrow(tmat_dt[abs(trat)>tr])/S
}
